...
async function main() {
  ...
  // shawdow session
  if (isActive) {
    await playSessionShadowing(gatewayAccessApi, activeFileType);
  } else {
    await playStaticRecording(gatewayAccessApi);
  }
}

async function playSessionShadowing(gatewayAccessApi, activeFileType) {
  try {
    const fileType = activeFileType || getFileType(await gatewayAccessApi.fetchRecordingInfo());
...
function getSessionDetails() {
  ...
  const activeFileType = windowURL.searchParams.get('activeFileType');
  const language = windowURL.searchParams.get('lang');

  return { sessionId, token, gatewayAccessUrl, isActive, activeFileType, language };
}
