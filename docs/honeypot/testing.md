...
- The canonical row `706` evidence gate is now `testsuite::honeypot_control_plane::verify_row706_evidence_envelope`, which reads the structured per-anchor fragments emitted by the four existing proof anchors under `target/row706/`.
...
- The verifier only accepts row `706` evidence when all four fragments are present, all required positive anchors are `executed=true` and `passed`, the negative control is `executed=true` and `passed`, and the positive anchors agree on `attestation_ref`, `base_image_path`, and `image_store_root`.
- Skipped `lab-e2e` anchors now record explicit `executed=false` row-`706` fragments, which means the verifier fails closed instead of letting old or partial artifacts masquerade as live Tiny11 proof.
