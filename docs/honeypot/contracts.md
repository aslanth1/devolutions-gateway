...
- `gateway.honeypot.command.approve` may reach only the non-executing deferred voting, keyboard, and clipboard placeholders in MVP.
...
- The `approve` role maps to `gateway.honeypot.command.approve` and may record only deferred or rejected placeholder votes plus disabled keyboard and clipboard placeholder requests in MVP.
...
## Keyboard Capture Placeholder

- The keyboard placeholder path is `POST /jet/session/{session_id}/keyboard` on `proxy` and `POST /session/{session_id}/keyboard` on `frontend`.
- The frontend route is HTMX-friendly form submission that relays to the proxy placeholder.
- The request fields are `schema_version`, `request_id`, and `key_sequence`.
- The success fields are `schema_version`, `correlation_id`, `capture_id`, `recorded_at`, `session_id`, `requested_key_count`, `capture_state`, `decision_reason`, and `executed`.
- `capture_state` is `disabled_by_policy` in MVP.
- `requested_key_count` reflects only the submitted key count and must not echo the raw keyboard payload back to the browser, logs, or audit text.
- `executed` is always `false` in MVP.
- The placeholder must record only a typed placeholder identifier and redacted request metadata, but it must not inject keystrokes, persist raw keyboard payloads, or mutate the VM.
- The placeholder requires `gateway.honeypot.command.approve`.

## Clipboard Capture Placeholder

- The clipboard placeholder path is `POST /jet/session/{session_id}/clipboard` on `proxy` and `POST /session/{session_id}/clipboard` on `frontend`.
- The frontend route is HTMX-friendly form submission that relays to the proxy placeholder.
- The request fields are `schema_version`, `request_id`, and `clipboard_text`.
- The success fields are `schema_version`, `correlation_id`, `capture_id`, `recorded_at`, `session_id`, `requested_byte_count`, `capture_state`, `decision_reason`, and `executed`.
- `capture_state` is `disabled_by_policy` in MVP.
- `requested_byte_count` reflects only the submitted clipboard payload size and must not echo the raw clipboard payload back to the browser, logs, or audit text.
- `executed` is always `false` in MVP.
- The placeholder must record only a typed placeholder identifier and redacted request metadata, but it must not inject clipboard text, persist raw clipboard payloads, or mutate the VM.
- The placeholder requires `gateway.honeypot.command.approve`.
...
- Frontend proposal, vote, keyboard, and clipboard placeholders are auditable by their typed `proposal_id`, `vote_id`, or `capture_id` plus `correlation_id`, `session_id`, and `decision_reason`, but they must remain non-executing and non-persistent in MVP.
