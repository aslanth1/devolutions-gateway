...
#[tokio::test]
async fn frontend_clipboard_capture_route_returns_disabled_placeholder() {
    let session_id = Uuid::new_v4().to_string();
    let (
        proxy_addr,
        proxy_handle,
        ...
        BootstrapResponse {
            schema_version: honeypot_contracts::SCHEMA_VERSION,
            correlation_id: "bootstrap-clipboard".to_owned(),
            generated_at: "2026-03-26T12:00:00Z".to_owned(),
            replay_cursor: "24".to_owned(),
            sessions: vec![BootstrapSession {
                session_id: session_id.clone(),
                vm_lease_id: Some("lease-clipboard".to_owned()),
                state: SessionState::Ready,
                last_event_id: "event-clipboard".to_owned(),
                last_session_seq: 4,
                stream_state: StreamState::Ready,
                stream_preview: None,
            }],
    ...
    let clipboard_path = authed_path(
        format!("/session/{session_id}/clipboard").as_str(),
        HONEYPOT_COMMAND_APPROVE_SCOPE_TOKEN,
    );
    let body = b"clipboard_text=secret-token-42";
    let (status_line, _headers, body) = send_http_request(
        port,
        "POST",
        &clipboard_path,
    ...
    .await
    .expect("read clipboard placeholder");
    let body = String::from_utf8(body).expect("decode clipboard placeholder html");

    assert!(status_line.contains("200"), "{status_line}");
    assert!(body.contains("Clipboard capture disabled"), "{body}");
    assert!(body.contains("disabled_by_policy"), "{body}");
    assert!(!body.contains("secret-token-42"), "{body}");
    assert!(
        observed_tokens
            .lock()
            .await
            .iter()
            .any(|entry| entry == &format!("CLIPBOARD:{session_id}:15"))
    );
...
#[tokio::test]
async fn frontend_dashboard_bootstraps_first_live_session_when_multiple_live_sessions_exist() {
    ...
    let (
        ...
        BootstrapResponse {
            schema_version: honeypot_contracts::SCHEMA_VERSION,
            correlation_id: "bootstrap-multi-focus".to_owned(),
            generated_at: "2026-03-30T14:00:00Z".to_owned(),
            ...
            sessions: vec![
                BootstrapSession {
                    ...
                    stream_preview: Some(StreamPreview {
                        ...
                        token_expires_at: "2026-03-30T14:05:00Z".to_owned(),
                    }),
                },
                BootstrapSession {
    ...
    assert!(status_line.contains("200"), "{status_line}");
    assert!(body.contains(&first_session_id), "{body}");
    assert!(body.contains(&second_session_id), "{body}");
    assert!(
        body.contains(&format!("const bootSessionId = \"{first_session_id}\";")),
        "{body}"
    );
    assert!(
        !body.contains(&format!("const bootSessionId = \"{second_session_id}\";")),
        "{body}"
    );
    assert!(body.contains("refreshFocus(bootSessionId)"), "{body}");
...
async fn start_mock_proxy_on_addr(
    ...
    let router = Router::new()
        ...
        .route("/jet/session/{id}/clipboard", post(mock_clipboard))
...
async fn mock_clipboard(
    ...
    state
        ...
        .lock()
        .await
        .push(format!("CLIPBOARD:{session_id}:{requested_byte_count}"));

    (
        StatusCode::OK,
        Json(ClipboardCaptureResponse {
            schema_version: honeypot_contracts::SCHEMA_VERSION,
            correlation_id: "clipboard-corr".to_owned(),
            capture_id: "clipboard-1".to_owned(),
            recorded_at: "2026-03-26T12:01:15Z".to_owned(),
            session_id,
