...
#[tokio::test]
async fn honeypot_keyboard_capture_route_is_disabled_by_default() -> anyhow::Result<()> {
    let config_handle = DgwConfig::builder()
        .disable_token_validation(true)
        .build()
        .init()
    ...
    let request = serde_json::to_vec(&KeyboardCaptureRequest {
        schema_version: honeypot_contracts::SCHEMA_VERSION,
        request_id: "keyboard-default-disabled".to_owned(),
        key_sequence: "abc".to_owned(),
    })
    .context("serialize keyboard request")?;
    let (status_line, _body) = send_http_request(
        config_handle.http_port(),
        "POST",
        format!("/jet/session/{}/keyboard", Uuid::new_v4()).as_str(),
        HONEYPOT_COMMAND_APPROVE_SCOPE_TOKEN,
        Some("application/json"),
        &request,
    )
...
#[tokio::test]
async fn honeypot_keyboard_capture_route_requires_approve_scope() -> anyhow::Result<()> {
    let config_handle = DgwConfig::builder()
        .disable_token_validation(true)
        .honeypot(HoneypotConfig::builder().enabled(true).build())
        .build()
    ...
    let request = serde_json::to_vec(&KeyboardCaptureRequest {
        schema_version: honeypot_contracts::SCHEMA_VERSION,
        request_id: "keyboard-forbidden".to_owned(),
        key_sequence: "abc".to_owned(),
    })
    .context("serialize keyboard request")?;
    let (status_line, _body) = send_http_request(
...
#[tokio::test]
async fn honeypot_keyboard_capture_route_returns_typed_disabled_placeholder_when_enabled() -> anyhow::Result<()> {
    let config_handle = DgwConfig::builder()
        .disable_token_validation(true)
        .honeypot(HoneypotConfig::builder().enabled(true).build())
        .build()
    ...
    let request = serde_json::to_vec(&KeyboardCaptureRequest {
        schema_version: honeypot_contracts::SCHEMA_VERSION,
        request_id: "keyboard-disabled".to_owned(),
        key_sequence: "abc123".to_owned(),
    })
    .context("serialize keyboard request")?;
    let (status_line, body) = send_http_request(
        config_handle.http_port(),
        "POST",
        format!("/jet/session/{session_id}/keyboard").as_str(),
        HONEYPOT_COMMAND_APPROVE_SCOPE_TOKEN,
        Some("application/json"),
        &request,
    )
    .await?;
    let response: KeyboardCaptureResponse = serde_json::from_slice(&body).context("parse keyboard capture response")?;
