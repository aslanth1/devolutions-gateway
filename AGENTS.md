...
Current evidence after the bounded `ironrdp-gfx` viability check: the current pinned IronRDP stack in this repo (`ironrdp-session`, `ironrdp-dvc`, `ironrdp-displaycontrol`, and `ironrdp-graphics`) exposes no obvious `RdpgfxClient`-style surface for a small client-only spike, so this tranche stopped before faking a graphics-on lane. The guacd review still reinforced the right shape for a future solution: graphics capability should be an explicit policy knob like guacd’s `enable_gfx`, not an inferred side effect or a guessed flag.
