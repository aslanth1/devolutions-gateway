...
- [x] `BS-23` Try an explicit `xfreerdp` `Rfx` or similarly scoped codec variant only after the control-minus result is archived.
...
Current evidence after the bounded `Rfx` contract proof: `testsuite/src/honeypot_manual_lab.rs` now exposes a small `render_manual_lab_xfreerdp_lane_contract` helper that renders the existing `xfreerdp-rfx` lane through the real `ManualLabXfreerdpGraphicsMode::Rfx` path, preserving `driver_lane=xfreerdp-rfx` plus `/dynamic-resolution` and `/gfx:RFX` without introducing a new reducer surface. `testsuite/tests/honeypot_manual_lab.rs` now proves those exact flags survive in per-session `driver_args` and that the archived same-day control companion still flows through `DGW_HONEYPOT_BS_CONTROL_ARTIFACT_ROOT` to a meaningful `control_run_comparison_summary`, while `docs/honeypot/runbook.md` names the same lane and archived-control prerequisite in the canonical black-screen contract.
...
Current evidence after the fresh same-day gate rerun: the refreshed opt-in proof `manual-lab-907a8f42279e43259b0dcf685d1b5549` again stayed truthful `503` during live observation and finished with wrapped-gfx `rdpgfx_dynamic_channel_open_count=0`, `rdpegfx_pdu_count=0`, zero emitted surface updates, and `black_screen_branch=negotiation_loss`, while the matched control `manual-lab-4e696bf3d1e8417587bc63232c6289ac` remained on the graphics path with `rdpegfx_pdu_count=462` and `black_screen_branch=player_loss`.

- [x] `BS-25` Add an opt-in IronRDP comparison lane with normal negotiated graphics.
...
Current evidence after the bounded `ironrdp-rdpgfx` contract proof: the testsuite now exposes a repo-owned `ManualLabIronRdpRdpgfxProbe` that attaches through `DrdynvcClient` on the opt-in `DGW_HONEYPOT_INTEROP_DRIVER_KIND=ironrdp-gfx` lane, emits `driver_lane=ironrdp-rdpgfx` plus `--rdpgfx`, advertises `CapabilitiesAdvertise` on `Microsoft::Windows::RDS::Graphics`, and frame-acknowledges server `EndFrame` PDUs in targeted tests without widening the persisted black-screen evidence contract. The guacd review still reinforced the right shape of the fix: graphics capability stays an explicit policy knob rather than an inferred side effect or a guessed label.
...
Current evidence after the fresh same-day gate rerun: the control proof `manual-lab-4e696bf3d1e8417587bc63232c6289ac` and the IronRDP no-gfx proof `manual-lab-907a8f42279e43259b0dcf685d1b5549` again emitted the same evidence families, including a non-`null` zero-object `fastpath_warning_summary` on the no-warning IronRDP lane. The only remaining path-level diff was the trivial “warn-code array contains an element versus is empty” case, which is value-level variance rather than contract drift.
...
- [x] `BS-29` Record the shadow websocket close reason and timing for every active-player failure.
...
- [x] `BS-31` Correlate session events with recording artifact creation and growth.
...
Current evidence after the explicit ready-slot proof: the two-session local manual-lab run `manual-lab-7466138d191b431ea011ddc06b5cc282` persisted `session-events.json` plus `playback_artifact_timeline_summary` in `black-screen-evidence.json`. Ready slot `cad257ff-9254-4155-8313-5af848f8a488` recorded `session_started_at_unix_ms=1774775207300`, `session_assigned_at_unix_ms=1774775226576`, `session_stream_ready_at_unix_ms=1774775251972`, `websocket_open_at_unix_ms=1774775630178`, `websocket_first_message_at_unix_ms=1774775630181`, and a growing `recording-0.webm` with `recording_artifact_max_size_bytes=404780`, yielding verdict `correlated_ready_playback`; sibling slot `d953c09d-f4cb-4210-bd04-795ccf6fec3e` preserved the matching `session.stream.failed` branch on the same run timeline.

- [x] `BS-32` Add a first-non-black-frame check for recording artifacts.
...
Current evidence after the repaired one-session proof: the local manual-lab run `manual-lab-70c3636dba2f41d584f258f8c2669598` persisted `recording_visibility_summary` in `black-screen-evidence.json` plus a session-local `recording-visibility-summary.json` beside `recording-0.webm`. Attached session `f43795cb-b8f5-4079-aff5-15c3158f4ed7` produced a `483K` WebM artifact, the headless Chrome reducer sampled `33` frames over `8000ms`, and the final verdict was `sparse_pixels` with `confidence=high`, `first_sparse_offset_ms=501`, and `max_non_black_ratio_per_mille=7`, which closes the "all black vs stray pixels vs visible frame" gap for recorded artifacts.

- [x] `BS-33` Compare browser-visible corruption with the underlying recording artifact at the same point in time.
...
Current evidence after the direct-player proof: the one-session local manual-lab run `manual-lab-7174734b82e549ee8162b382e389d348` persisted `browser_visibility_summary`, `artifact_visibility_at_browser_time`, and `browser_artifact_correlation_summary` in `black-screen-evidence.json`. Attached session `303f40b3-946e-40d8-b0d5-f8ce941d42ca` kept the active player in `dominant_mode=active_live`, selected a steady browser window with verdict `all_black` at `representative_current_time_ms=1`, and the aligned seek probe against `recording-0.webm` also returned `all_black`, which reduced to `browser_artifact_correlation_summary.verdict=both_black` instead of another teardown-time parse failure.

- [x] `BS-34` Add a positive ready-path test that proves active playback does not immediately collapse into static fallback.
...
Current evidence after the targeted ready-path guard: the repo now has `manual_lab_ready_path_sustain_accepts_steady_active_live_window` and `manual_lab_ready_path_sustain_rejects_static_fallback_before_steady_window` in `testsuite/tests/honeypot_manual_lab.rs`, backed by the reusable `build_manual_lab_ready_path_sustain_summary` reducer in `testsuite/src/honeypot_manual_lab.rs`. The positive case requires `AlignedReady`, `ActiveLivePath`, and a ready `steady` browser window in `active_live` mode with no `static_playback_started` evidence, which turns the old immediate-fallback regression into a direct failing contract instead of a narrative claim.

- [x] `BS-35` Add a multi-session ready-path proof that includes the historically weak third slot.
...
Current evidence after the slot-accounting reducer: `testsuite/src/honeypot_manual_lab.rs` now persists `multi_session_ready_path_summary` inside `ManualLabBlackScreenEvidence`, backed by `build_manual_lab_multi_session_ready_path_summary`, which reduces slots `1..=session_count` into named outcomes such as `usable_live_playback`, `static_fallback_observed`, `telemetry_gap`, or `missing_slot_evidence` instead of reporting only aggregate narrative state. `testsuite/tests/honeypot_manual_lab.rs` now includes `manual_lab_multi_session_ready_path_summary_accounts_for_three_slots` and `manual_lab_multi_session_ready_path_summary_marks_missing_slot_evidence`, which prove that slot three is accounted for explicitly and that missing slot evidence fails loudly rather than disappearing from the sanctioned proof flow.
...
- [x] `BS-37` Add a do-not-retry ledger for disproven hypotheses.
...
Current evidence after the do-not-retry ledger reducer: `testsuite/src/honeypot_manual_lab.rs` now persists a JSON-owned `do_not_retry_ledger` inside `ManualLabBlackScreenEvidence`, backed by explicit `hypothesis_id` and `hypothesis_text` inputs plus fixed retry-condition codes instead of another narrative tracker. The ledger stays record-only for `BS-37`, records the failing lane, artifact root, primary rejection reason, and machine-checkable retry condition for non-green runs, and `testsuite/tests/honeypot_manual_lab.rs` now proves amber entry-recorded, red entry-recorded, green not-required, and missing-retry-condition fail-closed outcomes through the real `integration_tests` harness.

- [x] `BS-38` Require a current control-run verdict before any new code or flag experiment is accepted.
